...
export default function AutoPaginate({
    ...
    useEffect(() => {
        // console.log("paginationObject:::", paginationObject, "current take:", currentTakeAmount, "calculated items:", (paginationObject?.to - paginationObject?.from) + 1);
    ...
    useEffect(() => {
        if (currentTakeAmount && ((currentTakeAmount - 1) > (paginationObject?.to - paginationObject?.from))) {
            // window.scrollTo({ left: 0, top: 0, behavior: "smooth" });;
            window.scrollTo({ left: 0, top: 0, behavior: "smooth" });
            // console.log("SCROLLED BOTTOM BASE");
        }
        //  else window.scrollTo({ left: 0, top: document.body.scrollHeight || document.documentElement.scrollHeight, behavior: "smooth" });
    }, [currentTakeAmount, paginationObject]);
                ...
                <ul className="flex">
                    <li>
                        <div onClick={() => {
                            ...
                            className={`h-s48 py-s10 px-s15 body_text text-cGray bg-cWhite rounded-l-br6 border border-cNmSelect
                        ...
                        <>
                            <li>
                                <div
                                    ...
                                    className={`h-s48  py-s10 px-s12 ${"text-cTextBody hover:bg-cGridView bg-cWhite hover:text-cTextShopDetails"} 
                            ...
                            {(paginationObject.current_page > 4) && (paginationObject.last_page > 5) ?
                                <li>
                                    <div
                                        ...
                                        className={`h-s48 border cursor-default py-s10 px-s12 text-cTextBody hover:bg-cGridView bg-cWhite hover:text-cTextShopDetails border-cNmSelect`}
                                    >
                                        ...
                                    </div>
                                </li> : ""}
                        ...
                        item.label !== 1 && item.label !== paginationObject?.last_page ?
                            <li key={index}>
                                <div
                                    ...
                                    className={`h-s48 py-s10 px-s12 body_text
                                        ${item?.active === true
                                            ? "text-cBrand hover:bg-cBorderBottom bg-cSelectedBar hover:text-cBrand"
                                            : "text-cGray hover:bg-cGridView bg-cWhite hover:text-cTextShopDetails"
                                        }   border border-cNmSelect cursor-pointer`}
                                >
                                    {item?.label}
                                </div>
                        ...
                        <>
                            ...
                            <li>
                                <div
                                    ...
                                    className={`py-s10 px-s12 ${"h-s48 text-cTextBody hover:bg-cGridView bg-cWhite hover:text-cTextShopDetails"} 
                    ...
                    <li>
                        <div onClick={() => {
                            ...
                            className={`h-s48 py-s10 px-s12 body_text text-cGray bg-white rounded-r-br6 border border-cNmSelect
